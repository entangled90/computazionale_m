...
void r_squared_save ( char * filename){
	...
	for ( delta = 1; delta  <  time_counted-1; delta++){
		...
		sum /= ((double) count*L*L);
		fprintf(f,"%e\t%e\n",delta*D_T*PERIOD_R2, sum);
		if (delta*PERIOD_R2 %10000 == 0){
			printf("Siamo a %d su %d in step\n",delta*PERIOD_R2,time_counted);
		}
	}
	fclose(f);
}
...
inline void energy_file_save(){
	FILE *f_energy = fopen("data/energy9_108.dat","a");
...
riscala_vel_temp();
create_box_file();
//FILE *f_energy = fopen("data/energy9_108.dat","w");
//fclose(f_energy);
